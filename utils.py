...
def crfrnn(ux, wsmooth, wcontra, k1, k2, trainiter=5, testiter=10, wunary=None): 
  ...
  if wunary is None:
    e_x = tf.exp(ux - tf.reduce_max(ux, reduction_indices=2, keep_dims=True))
  else:
    ux = tf.reshape(ux, [-1, 8])
...
def init(nhid1=6, nhid2=12, nhid3=588, lrf1=5, lrf2=5, lrf3=7):
...
def crfatmodel(X, Y, k1, k2, paras, epsilon, flag='single'):
  assert(flag=='single' or flag=='combine')
  energy, accuracy, di, testenergy, predarg = model(X, Y, k1, k2, paras, flag)
  ...
  return energy, accuracy, di, testenergy, predarg, advenergy

def cnnatmodel(X, Y, paras, epsilon, flag='single'):
  assert(flag=='single' or flag=='combine')
  energy, accuracy, di, testenergy, predarg = cnnmodel(X, Y, paras, flag=flag)
  gradx = tf.stop_gradient(tf.gradients(energy, X, aggregation_method=2))
  gradx = gradx / (1e-6 + tf.reduce_max(tf.abs(gradx), reduction_indices=[1,2], keep_dims=True))
  gradx = gradx / tf.sqrt(1e-12 + tf.reduce_sum(gradx**2, reduction_indices=[1,2], keep_dims=True))
  radv = epsilon * gradx
